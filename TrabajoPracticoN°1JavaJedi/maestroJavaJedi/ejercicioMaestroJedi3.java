public class ejercicioMaestroJedi3 {
    public static void main(String[] args) {
        /*. Cálculo de la hipotenusa de un triángulo:
        Escribir un programa en Java que calcule la hipotenusa de un triángulo rectángulo,
        dados los valores de los catetos. Utilizar la fórmula: 
        h^2 = a^2 + b^2, donde h es la
        hipotenusa, a y b son los catetos */
        double catetoA= 3;
        double catetoB = 3;
        double resultado = catetoA * catetoA + catetoB * catetoB;
        System.out.println("La hipotenusa de un triangulo rectangulo en este caso es "+ resultado);

    }
}
